...
internal sealed class ArtworkReviewForm : Form
{
    ...
    public ArtworkReviewForm(SteamGridDbClient client, GameEntry game)
    {
        ...
        Text = $"Artwork review – {game.Name}"; Width = 970; Height = 530; MinimumSize = new Size(800, 450); StartPosition = FormStartPosition.CenterParent;
        ...
        cards.Controls.Add(CreateCard("Cover", _cover, ArtworkKind.Cover), 0, 0);
        cards.Controls.Add(CreateCard("Background / hero", _hero, ArtworkKind.Hero), 1, 0);
        cards.Controls.Add(CreateCard("Logo", _logo, ArtworkKind.Logo), 2, 0);
        var approve = new Button { Text = "Approve and continue", AutoSize = true, DialogResult = DialogResult.OK };
        var cancel = new Button { Text = "Cancel import", AutoSize = true, DialogResult = DialogResult.Cancel };
